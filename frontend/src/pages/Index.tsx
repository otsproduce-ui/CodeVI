...
export default function Index() {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      const { scanCodebase } = await import("@/lib/api");
      const result = await scanCodebase(rootPath || ".");
      setIsScanning(false);
      setStatus({ 
        type: "success", 
        message: `Found ${result.count} files indexed successfully!` 
      });
    } catch (err) {
      setIsScanning(false);
      setStatus({ 
        type: "error", 
        message: err instanceof Error ? err.message : "Failed to scan codebase" 
      });
    }
  };
